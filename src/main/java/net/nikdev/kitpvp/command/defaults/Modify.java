...
public class Modify implements Argument {
    ...
    public void execute(CommandSender sender, String[] args) throws CommandException {
        if(args.length != 4) {
            Lang.sendTo(sender, Keys.NO_TOKEN_ARGS);
        ...
        if(!target.isPresent()) {
            throw new CommandException(Lang.get(Keys.ERROR_PARSE_TARGET, Placeholder.of("target", args[2])));
        }
        ...
        try {
            ...
            throw new CommandException(Lang.get(Keys.ERROR_PARSE_AMOUNT, Placeholder.of("amount", args[3])));
        }

        switch(args[1]) {
            case "add":
                target.get().getStats().addTokens(amount);
                Lang.sendTo(sender, Keys.TOKENS_UPDATED, Placeholder.of("target", args[2]), Placeholder.of("amount", target.get().getStats().getTokens()));
            ...
            default:
                throw new CommandException(Lang.get(Keys.ERROR_UNKNOWN_MODIFY, Placeholder.of("operation", args[1])));
        }
